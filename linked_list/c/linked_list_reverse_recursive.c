...
SLLNode *reverse_linked_list_recursive(SLLNode *head){

    if(head == NULL || head->next == NULL)
      return head;

    SLLNode *next_head = reverse_linked_list_recursive(head->next);
    head->next->next = head;
    head->next = NULL;
    return next_head;
}
...
int main(){
    ...
    printf("\nLinked List after executing the reverse function (recursive)");
    print_linked_list(head);
